"""
LSTM版 PIDゲイン推論コントローラー。

LearnedPIDONNXController (MLP版) との唯一の違いは _build_window_input() の
出力形状:
  MLP版: (1, history * frame_dim)   flat
  LSTM版: (1, history, frame_dim)   3D  ← LSTMが期待する形状

それ以外 (特徴量生成・PID計算・ゲイン平滑化・操舵レート制限・モーター平滑化) は
LearnedPIDONNXController と同一。
"""
...
class LSTMPIDONNXController:
    ...
    def _build_window_input(self) -> np.ndarray:
        """
        MLP版との唯一の違い。
        MLP版: flat.reshape(1, history * frame_dim)     → (1, 120)
        LSTM版: np.stack(...).reshape(1, history, dim)  → (1, 10, 12)
        """
        ...
        seq = np.stack(history_list, axis=0).astype(np.float32)  # (history, frame_dim)
        return seq.reshape(1, self.history, frame_dim)            # (1, history, frame_dim)

    def _pid_terms_from_window(self, seq: np.ndarray) -> Tuple[float, float, float, float, float]:
        """seq: shape (history, frame_dim)"""
        frame_dim = len(self.FEATURE_KEYS) + 3
        seq = np.asarray(seq, dtype=np.float32).reshape(self.history, frame_dim)
    ...
    def predict_motor_speed(
        ...
        inp = self._build_window_input()                           # (1, history, frame_dim)
        seq_2d = inp.reshape(self.history, -1)                     # (history, frame_dim) for PID calc
        error, integral, derivative, base_speed_norm, detect_sum = self._pid_terms_from_window(seq_2d)
        ...
        if self.stop_on_no_line and detect_sum <= 0.0:
            ...
            if brake_step <= self.no_line_brake_frames:
                ...
                return self._norm_to_speed(self._left_norm_prev * scale, self._right_norm_prev * scale)
        ...
        self._net.setInput(inp)  # (1, history, frame_dim) をそのまま渡す
